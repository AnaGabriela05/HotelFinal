...
public class ReservaService {
	...
	public void registrarReserva(Reservas reserva, String id, Long idusuario) {
		...
		reposReservaRepository.registrarReserva(id, reserva.getFechaentrada(), reserva.getFechasalida(),
				reserva.getNombreuser(), reserva.getApellidouser(), reserva.getCorreouser(), reserva.getTelefonouser(),
				reserva.getIdhabitacion(), idusuario, "No confirmado", reserva.getNoche_reserva());
	}
	...
	public void actualizarReserva(Reservas reservas, String id) {
		reposReservaRepository.actualizarReserva(reservas.getIdreserva(), reservas.getFechaentrada(),
				reservas.getFechasalida(), reservas.getNombreuser(), reservas.getApellidouser(),
				reservas.getCorreouser(), reservas.getTelefonouser(), id, reservas.getIdusuario(), "No confirmado",
				reservas.getNoche_reserva());
	}

	public void actualizarReserva_sin_habitacion(Reservas reservas) {
		reposReservaRepository.actualizarReserva(reservas.getIdreserva(), reservas.getFechaentrada(),
				reservas.getFechasalida(), reservas.getNombreuser(), reservas.getApellidouser(),
				reservas.getCorreouser(), reservas.getTelefonouser(), reservas.getIdhabitacion(),
				reservas.getIdusuario(), "No confirmado", reservas.getNoche_reserva());
	}
